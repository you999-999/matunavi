/**
 * まつなび - 利用規約ページ（Terms of Service）
 * 
 * このファイルは、まつなびの利用規約を表示する法的文書ページです。
 * 
 * 【目的】
 * - サービス利用の条件を明示
 * - 医療行為ではないことを法的に明記
 ...
 * 
 * 【規約の構成（全9条）】
 * 第1条: サービス内容 - 医療行為ではないことを明示
 * 第2条: 医療に関する免責 - 診断・治療は医師に相談する旨
 * 第3条: 情報の正確性 - 内容を保証しないことを明記
 * 第4条: 禁止事項 - 不正アクセス、妨害行為などを禁止
 * 第5条: サービスの変更・停止 - 事前通知なく変更可能
 * 第6条: 責任の制限 - 損害に対する免責
 * 第7条: 著作権 - コンテンツの権利帰属
 * 第8条: 規約の変更 - 規約変更の権利
 * 第9条: 準拠法 - 日本法を適用
 ...
 * - ユーザーとの法的関係の明確化
 * - 医療情報サービスとしての責任範囲の限定
 * - AdSense審査でのコンプライアンス対応
 * 
 * 【デザインパターン】
 * - 見出しは h1（タイトル）、h2（各条文）
 * - 読みやすさを重視した段落分け
 * - セクションごとに <hr> で視覚的に区切り
 * - TOPページへ戻るボタンを配置
 * 
 * 【メンテナンス】
 * - 法改正時は内容の見直しが必要
